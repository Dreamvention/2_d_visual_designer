<wrapper-blocks><div class="vd-new-child-block" onClick={addBlock} if={_.isEmpty(getState().blocks) && !getState().drag}></div><div data-is="placeholder" if={getState().placeholder && !_.isEmpty(getState().blocks)} placeholder_type={getState().placeholder_type} sort_order="0" block_id={getState().parent_id}/><virtual each={block in getState().blocks}><div data-is="vd-layout-{block.layout}" block={block}></div><div data-is="placeholder" placeholder_type={getState().placeholder_type} sort_order={block.sort_order+1} block_id={getState().parent_id} if={getState().placeholder}/>
</virtual><div data-is="placeholder" if={getState().placeholder && _.isEmpty(getState().blocks)} placeholder_type={getState().placeholder_type} sort_order="0" block_id={getState().parent_id}/>
<script>
