<vd-layout-medium class="block-inner block-container {className} {activeControl? 'active-control':'deactive-control'}" data-id="{opts.block.id}" id={block_setting.id? block_setting.id:null}>
    <virtual if={permission}>
        <div class="block-mouse-toggle"></div>
        <div class="control control-{block_config.setting.control_position} {upControl? 'control-up': ''}">
