<vd-layout-medium class="block-inner block-container {getState().className} {getState().activeControl? 'active-control':'deactive-control'}" data-id="{opts.block.id}" id={getState().setting.id? getState().setting.id:null}>
    <virtual if={getState().permission}>
        <div class="block-mouse-toggle"></div>
        <div class="control control-{getState().block_config.setting.control_position} {getState().upControl? 'control-up': ''}">
