<vd-layout-block-row_inner>
    <div class="row layout-edit">
        <div each={value in store.getOptions('blocks.row_inner.layout_sizes')} class="col-md-3 col-sm-6 col-xs-12 layout {size == value ? 'active' : ''}" onClick={editLayout}>
            <a id="edit-layout">
    ...
    <div class="setting">
        <div class="form-group">
            <label class="control-label">{store.getLocal('blocks.row_inner.entry_size')}</label>
            <div class="fg-setting">
                <div class="input-group">
                    <input type="text" class="form-control" name="size" value="{size}"/>
                    <span class="input-group-btn">
                        <button id="layoutSet" class="btn btn-default" type="button" onClick={setLayout}>{store.getLocal('blocks.row_inner.text_set_custom')}</button>
                    </span>
                </div>
            </div>
        </div>
